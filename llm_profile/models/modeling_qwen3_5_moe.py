...
try:
    ...
    def use_experts_implementation(cls): return cls
...
try:
    ...
    def torch_compilable_check(fn): return fn
try:
...
try:
    ...
    get_vision_bilinear_indices_and_weights = None
    get_vision_cu_seqlens = None
    get_vision_position_ids = None
...
class Qwen3_5MoeGatedDeltaNet(nn.Module):
    def __init__(self, config: Qwen3_5MoeConfig, layer_idx: int):
        ...
        # Linear-attention (gated delta net) timers — bundled to MLP-level granularity (3 timers)
        # 3/4 of layers use this path so coverage matters more than fine breakdown.
        self._gdn_in_proj_timer = Timer(name="gdn_in_proj")  # 4 Linears: qkv + z + b + a
        self._gdn_kernel_timer = Timer(name="gdn_kernel")    # conv1d + delta_rule + prep
        self._gdn_post_timer = Timer(name="gdn_post")        # RMSNormGated + out_proj
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        # Update cache (not timed — state mgmt)
        if cache_params is not None:
            cache_params.update_recurrent_state(last_recurrent_state, self.layer_idx)

        with self._gdn_post_timer:
...
class Qwen3_5MoeAttention(nn.Module):
    ...
    def __init__(self, config: Qwen3_5MoeConfig, layer_idx: int):
        ...
        self.is_causal = True
        tp = getattr(config, "tp_size", 1)
        # q_proj outputs (num_attention_heads * head_dim * 2) to split into (query, gate); shard heads across TP
        self.q_proj = nn.Linear(
            config.hidden_size, config.num_attention_heads * self.head_dim * 2 // tp, bias=config.attention_bias
        )
        self.k_proj = nn.Linear(
            config.hidden_size, config.num_key_value_heads * self.head_dim // tp, bias=config.attention_bias
        )
        self.v_proj = nn.Linear(
            config.hidden_size, config.num_key_value_heads * self.head_dim // tp, bias=config.attention_bias
        )
        self.o_proj = nn.Linear(
            config.num_attention_heads * self.head_dim // tp, config.hidden_size, bias=config.attention_bias
        )
...
class Qwen3_5MoeMLP(nn.Module):
    """Used as the shared_expert in the Sparse MoE block."""
    def __init__(self, config: Qwen3_5MoeConfig, intermediate_size: int):
        super().__init__()
        self.config = config
        self.hidden_size = config.hidden_size
        tp = getattr(config, "tp_size", 1)
        self.intermediate_size = intermediate_size // tp
        self.gate_proj = nn.Linear(self.hidden_size, self.intermediate_size, bias=False)
        self.up_proj = nn.Linear(self.hidden_size, self.intermediate_size, bias=False)
        self.down_proj = nn.Linear(self.intermediate_size, self.hidden_size, bias=False)
        self.act_fn = ACT2FN[config.hidden_act]

        self._shared_expert_mlp_timer = Timer(name="shared_expert.mlp")

    def forward(self, x):
        with self._shared_expert_mlp_timer:
...
class Qwen3_5MoeExperts(nn.Module):
    ...
    def __init__(self, config):
        ...
        # moe_intermediate_size shards across TP ranks (gate_up_proj: packed_colwise; down_proj: rowwise)
        self.intermediate_dim = config.moe_intermediate_size // tp
...
class Qwen3_5MoeTopKRouter(nn.Module):
    def __init__(self, config):
        ...
        self._router_timer = Timer(name="router")
...
class Qwen3_5MoeTextModel(Qwen3_5MoePreTrainedModel):
    ...
    def __init__(self, config: Qwen3_5MoeTextConfig):
        ...
        tp = getattr(config, "tp_size", 1)
        self.embed_tokens = nn.Embedding(config.vocab_size // tp, config.hidden_size, config.pad_token_id)
...
class Qwen3_5MoeForCausalLM(Qwen3_5MoePreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config):
        ...
        self.lm_head = nn.Linear(config.hidden_size, config.vocab_size // tp, bias=False)
