...
class Qwen3MLP(nn.Module):
    def __init__(self, config):
        ...
        self.gate_proj = nn.Linear(self.hidden_size, self.intermediate_size // config.tp_size, bias=False)
        self.up_proj = nn.Linear(self.hidden_size, self.intermediate_size // config.tp_size, bias=False)
        self.down_proj = nn.Linear(self.intermediate_size // config.tp_size, self.hidden_size, bias=False)
    ...
    def forward(self, x):
        ...
        with self._down_proj_timer:
            down_proj = self.down_proj(act_fn)
        return down_proj
...
class Qwen3Attention(nn.Module):
    ...
    def __init__(self, config: Qwen3Config, layer_idx: int):
        ...
        self.q_norm = Qwen3RMSNorm(self.head_dim // config.tp_size, eps=config.rms_norm_eps)  # unlike olmo, only on the head dim!
        self.k_norm = Qwen3RMSNorm(self.head_dim // config.tp_size, eps=config.rms_norm_eps)  # thus post q_norm does not need reshape
    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        hidden_shape_tp = (*input_shape, -1, self.head_dim // self.config.tp_size)

        with self._q_proj_timer:
            query_states = self.q_norm(self.q_proj(hidden_states).view(hidden_shape_tp)).view(hidden_shape).transpose(1, 2)
        with self._k_proj_timer:
            key_states = self.k_norm(self.k_proj(hidden_states).view(hidden_shape_tp)).view(hidden_shape).transpose(1, 2)
        with self._v_proj_timer:
            value_states = self.v_proj(hidden_states).view(hidden_shape).transpose(1, 2)

        cos, sin = position_embeddings
        #cos = cos[:, :, :self.head_dim // self.config.tp_size]  # 계산정확도 x
        #sin = sin[:, :, :self.head_dim // self.config.tp_size]  # 계산정확도 x
